module Unit.Hoard.Listeners.MonitoringListenerSpec (spec_MonitoringListener) where

import Data.Default (def)
import Data.UUID qualified as UUID
import Effectful (runPureEff)
import Effectful.State.Static.Shared (evalState)
import Effectful.Writer.Static.Shared (execWriter)
import Test.Hspec (Spec, it, shouldBe)
import Prelude hiding (evalState)

import Hoard.Data.ID (ID (..))
import Hoard.Data.Peer (Peer (..))
import Hoard.Effects.Log (runLogWriter)
import Hoard.Events.MonitoringRequest (MonitoringRequest (..))
import Hoard.Listeners.MonitoringListener (monitoringListener)
import Hoard.Types.HoardState (HoardState (..))


spec_MonitoringListener :: Spec
spec_MonitoringListener = do
    it "reports correct number of peers" do
        let logs =
                runPureEff
                    . execWriter @[Text]
                    . runLogWriter
                    . evalState
                        ( def
                            { connectedPeers = fromList $ mkPeerIDs 3
                            }
                        )
                    $ monitoringListener MonitoringRequest
        logs `shouldBe` ["[INFO] Currently connected to 3 peers"]


mkPeerIDs :: Int -> [ID Peer]
mkPeerIDs n =
    [ ID $ UUID.fromWords64 (fromIntegral i) (fromIntegral i + 1)
    | i <- [0 .. (n - 1)]
    ]
