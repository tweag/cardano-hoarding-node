...
import Effectful.Concurrent (myThreadId)
...
registerRequest config headers = do
    stateM \s ->
        ...
        in
            do
                threadId <- myThreadId
                when (bytesInFlight /= newBytes) $
                    Log.debug $
                        "BLOCK FETCH IS CONCURRENT register: threadId " <> show threadId <> ", current bytes in flight " <> show bytesInFlight <> ", registering " <> show newBytes <> " bytes"
...
unregisterRequest config headers = do
    stateM \s ->
        ...
        in
            do
                threadId <- myThreadId
                when (bytesInFlight /= 0) $
                    Log.debug $
                        "BLOCK FETCH IS CONCURRENT unregister: threadId " <> show threadId <> ", current bytes in flight " <> show bytesInFlight <> ", unregistering " <> show newBytes <> " bytes"
