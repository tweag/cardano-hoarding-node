...
waitUntilReadyToFetch :: (Concurrent :> es, State Status :> es, Log :> es) => Eff es ()
waitUntilReadyToFetch = do
    Log.debug "Waiting until ready to issue block fetch request..."
    _ <- readMVar =<< gets (.stateVar)
    Log.debug "Ready to issue block fetch request!"
        ...
        in
            do
                when (coerce bytesInFlight > config.highWatermark) $ do
                    ...
                    when (isJust x) $
                        Log.debug "BlockFetch.State: entering busy state"
        ...
        in
            do
                when (coerce bytesInFlight < config.lowWatermark) $ do
                    ...
                    when wasBusy $
                        Log.debug "BlockFetch.State: entering ready state"
                pure ((), s {bytesInFlight})
