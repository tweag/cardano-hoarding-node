...
import Effectful.Dispatch.Dynamic (localSeqUnlift, reinterpretWith)
...
runSingleflight action = do
    ...
    reinterpretWith (Reader.runReader cache) action $ \env -> \case
            ...
            (mvar, isFirst) <- STM.atomically $ do
                ...
                case Map.lookup key cache' of
                    ...
                    Nothing -> do
                        ...
                        STM.writeTVar cacheVar (Map.insert key tmvar cache')
                            ...
                            then do
                                ...
                                case result of
                                    Left _exception -> do
                                        STM.atomically $ do
                                            cache' <- STM.readTVar cacheVar
                                            STM.writeTVar cacheVar (Map.delete key cache')
                                    Right _ -> pure ()
                ...
                forM_ entries $ \(key, value) -> do
                    case Map.lookup key cache' of
                        ...
                        Nothing -> do
                            ...
                            STM.modifyTVar cacheVar (Map.insert key tmvar)
