module Hoard.ChainSync.NodeToNode (miniProtocol, client, limits) where
...
import Ouroboros.Network.NodeToNode (MiniProtocolParameters (..), MinimalInitiatorContext, ResponderContext, chainSyncMiniProtocolNum, defaultMiniProtocolParameters)
...
limits =
    MiniProtocolLimits
        { maximumIngressQueue = fromIntegral $ defaultMiniProtocolParameters.chainSyncPipeliningHighMark * 4
        }
