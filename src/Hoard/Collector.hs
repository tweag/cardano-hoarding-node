...
import Hoard.Network.Config (NetworkConfig, defaultNetworkConfig)
...
dispatchDiscoveredNodes = listen $ \(NodeDiscovered peer) ->
    fork_ $ runCollector defaultNetworkConfig peer
...
runCollector
    ...
    -> Peer
    -> Eff es Void
runCollector networkCfg peer = do
    publish $ CollectorStarted peer
    publish $ ConnectingToPeer peer

    -- Connect to peer (errors will propagate via Error effect)
    _conn <- connectToPeer networkCfg peer
