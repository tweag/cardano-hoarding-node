...
import Hoard.Effects.Verifier (Verifier, getVerified, verifyHeader)
...
pickBlockFetchRequest myPeerId event =
    unless (event.peer.id /= myPeerId) $
        verifyHeader event.header >>= \case
            Left _invalidHeader -> do
                addEvent "collector_invalid_header" [("hash", show event.header.hash)]
